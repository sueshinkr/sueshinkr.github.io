...
##### 내멋대로 해석    
:  src에 저장되어있는 값을 n만큼 복사해 dest의 시작주소부터 붙여넣는다. 각 메모리가 overlap되어있어서는 안되며, 그럴 경우엔 memcpy 대신 memmove를 사용해야한다. 리턴값은 dest.  
...
##### 의문점 및 생각해볼점    
1. [overlap?](#overlap이란)    
2. [복사할 size가 dest의 크기를 넘어간다면?](#dest의-크기보다-더-큰-값을-복붙할경우)
...
이런식으로 존재할경우 src의 값을 dest에 하나씩 복붙하다보면    
...
## dest의 크기보다 더 큰 값을 복붙할경우

그냥 dest의 크기에 관계없이 복붙하겠다는 메모리 그대로 dest에 넣어버려 에러 혹은 엉뚱한 결과가 발생했다.
...
```
memset함수에서 dest와 src 둘 다 NULL일때는 NULL값이 반환된다고 적힌 글들도 있던데, 일단 내가 본 메뉴얼상에는 그런 내용이 없었으니 취급하지 않기로 결정했다. 특별히 protect하는 것보단 그냥 에러뜨게 냅두자는 생각이다.
